...
public class NCHAdapter extends RecyclerView.Adapter<NCHAdapter.ViewHolder> {
    private final Activity context;
    Database dbHandler;
    private List<House> data;
    String env;

    public NCHAdapter(Activity context, List<House> list, Database dbHandler, String env){
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        final House a=data.get(position);
        holder.Srno.setText(String.valueOf(a.getNchid()));

        holder.Street.setText(a.area_name);
        holder.HHs.setText(String.valueOf(a.getHHs()));
        int listed=dbHandler.queryInteger("SELECT count(*) from "+House.class.getSimpleName()+" where ENV='"+env+"' AND  is_deleted=0 and nchid="+a.getNchid());
        if(listed==0){
            holder.imgtick.setVisibility(View.INVISIBLE);
        }
        if(a.sync_time!=null && a.getHHs()!=0){
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            HHs=itemView.findViewById(R.id.txt_hh);
                    ...
                    public void onResponse(Call<Returning> call, Response<Returning> response) {
                        ...
                        if(u!=null){
                            ...
                            if(u.getCode()==1){
                                ...
                                ((NCHListActivity)context).updateUpload();
    ...
    public String withinDate(House a){
        try{
            String sdate=dbHandler.queryString("SELECT start_date from "+ NCH.class.getSimpleName()+" where (is_deleted='false' or is_deleted=0) and id="+a.getNchid());
            String edate=dbHandler.queryString("SELECT end_date from "+ NCH.class.getSimpleName()+" where (is_deleted='false' or is_deleted=0) and id="+a.getNchid());
